...
Var CheckboxRPCRESTHOME
Var CheckboxRPCRESTHOME_State
...
Function VariablesEntornoPage
    ...
    ${NSD_CreateCheckbox} 10 20u 100% 12u "Set the RPCRESTHOME environment variable."
    Pop $CheckboxRPCRESTHOME
    ${If} $CheckboxRPCRESTHOME_State == ${BST_CHECKED}
        ${NSD_Check} $CheckboxRPCRESTHOME
    ${EndIf}
        
    ${NSD_CreateCheckbox} 10 32u 100% 12u "&Add to the PATH environment variable the location of eProsima RPCREST scripts"
    Pop $CheckboxScripts
    ${If} $CheckboxScripts_State == ${BST_CHECKED}
        ${NSD_Check} $CheckboxScripts
    ${EndIf}

    ${NSD_CreateCheckbox} 10 44u 100% 24u "&Add to the PATH environment variable the location of eProsima RPCREST target$\r$\nlibraries for platform i86Win32VS2010"
    Pop $CheckboxI86VS2010

    ${NSD_CreateCheckbox} 10 66u 100% 24u "&Add to the PATH environment variable the location of eProsima RPCREST target$\r$\nlibraries for platform i86Win32VS2013"
    ...
    ${If} ${RunningX64}
        ${NSD_CreateCheckbox} 10 88u 100% 24u "&Add to the PATH environment variable the location of eProsima RPCREST target$\r$\nlibraries for platform x64Win64VS2010"
    ...
    ${If} ${RunningX64}
        ${NSD_CreateCheckbox} 10 110u 100% 24u "&Add to the PATH environment variable the location of eProsima RPCREST target$\r$\nlibraries for platform x64Win64VS2013"
...
### Callback invocado cuando se pulsa el CheckBox RPCRESTHOME
### Guardamos el estado en la variable _state
Function ClickRPCRESTHOME
    ${NSD_GetState} $CheckboxRPCRESTHOME $CheckboxRPCRESTHOME_State
FunctionEnd
