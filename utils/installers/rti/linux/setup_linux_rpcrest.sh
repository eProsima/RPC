...
function installer
{
    ...
    cp $EPROSIMADIR/code/eProsima_cpp/eProsima_auto_link.h tmp/$project/include/rpcrest/eProsima_cpp
    errorstatus=$?
    if [ $errorstatus != 0 ]; then return; fi
    cp $EPROSIMADIR/code/eProsima_cpp/eProsimaMacros.h tmp/$project/include/rpcrest/eProsima_cpp
    errorstatus=$?
    if [ $errorstatus != 0 ]; then return; fi
    ...
    cd tmp
    tar cvzf "../eProsima_RPCREST-${version}-Linux-RTIDDS-5.0.0.tar.gz" $project
    errorstatus=$?
    cd ..
    if [ $errorstatus != 0 ]; then return; fi
}
