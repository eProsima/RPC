...
function installer
{
    ...
    cp $EPROSIMADIR/code/eProsima_cpp/eProsima_auto_link.h tmp/$project/include/fastrpc/eProsima_cpp
    errorstatus=$?
    if [ $errorstatus != 0 ]; then return; fi
    cp $EPROSIMADIR/code/eProsima_cpp/eProsimaMacros.h tmp/$project/include/fastrpc/eProsima_cpp
    errorstatus=$?
    if [ $errorstatus != 0 ]; then return; fi
