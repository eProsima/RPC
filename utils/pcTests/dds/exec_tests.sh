...
if [ "$FASTCDR" == "" ]; then
    echo "FASTCDR environment variables is not set."
    exit -1
fi
        ...
        if [ -e "$dir/exec_test.sh" ] ; then
            if [ $errorstatus == 0 ]; then
                if [ -z $test_targets ] || [ "$test_targets" == "i86" ]; then
                    ...
                    . $EPROSIMADIR/scripts/common_exectest_functions.sh setTargetLibraryPath ../../../lib/$NDDSTARGET:$FASTCDR/fastcdr/lib/$EPROSIMA_TARGET
