...
function execTest
{
    ...
    ../../../scripts/fastrpcgen.sh -local -protocol dds -topicGeneration byOperation -d output -example $NDDSTARGET "$1/$1.idl"
    errorstatus=$?
    ...
    # Generates the file with FASTRPC script
    ../../../scripts/fastrpcgen.sh -local -protocol dds -topicGeneration byInterface -d output -example $NDDSTARGET "$1/$1.idl"
    errorstatus=$?
    if [ $errorstatus != 0 ]; then return; fi
    # TODO Temporary set of LIBFASTCDR
    export LIBFASTCDR=../../../../../fastcdr/lib/$EPROSIMA_TARGET
