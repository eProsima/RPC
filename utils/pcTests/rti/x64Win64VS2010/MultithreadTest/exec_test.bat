...
call ..\..\..\..\..\scripts\rpcdds_rti_pcTests.bat -ppDisable "%TEST_NAME%.idl"
...
msbuild "%TEST_NAME%-vs2010.sln" /t:Clean
:: Build the visual solution
msbuild "%TEST_NAME%-vs2010.sln" /t:Build /p:Configuration="Release DLL"
set errorstatus=%ERRORLEVEL%
if not %errorstatus%==0 goto :exit
...
msbuild "%TEST_NAME%-vs2010.sln" /t:Clean
:: Build the visual solution
msbuild "%TEST_NAME%-vs2010.sln" /t:Build /p:Configuration="Debug DLL"
set errorstatus=%ERRORLEVEL%
if not %errorstatus%==0 goto :exit
:: Execute the server in other cmd.exe
start objs\x64Win64VS2010\%TEST_NAME%Server.exe
...
msbuild "%TEST_NAME%-vs2010.sln" /t:Clean
:: Build the visual solution
msbuild "%TEST_NAME%-vs2010.sln" /t:Build /p:Configuration="Release"
set errorstatus=%ERRORLEVEL%
if not %errorstatus%==0 goto :exit
:: Execute the server in other cmd.exe
start objs\x64Win64VS2010\%TEST_NAME%Server.exe
