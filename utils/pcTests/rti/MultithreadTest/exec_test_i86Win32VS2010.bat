...
msbuild "%TEST_NAME%-i86Win32VS2010.sln" /t:Clean
:: Build the visual solution
msbuild "%TEST_NAME%-i86Win32VS2010.sln" /t:Build /p:Configuration="Release DLL"
set errorstatus=%ERRORLEVEL%
if not %errorstatus%==0 goto :exit
:: Execute the server in other cmd.exe
start objs\i86Win32VS2010\%TEST_NAME%Server.exe
:: Wait 5 seconds
call :wait 5
:: Execute the client in this cmd.exe
"objs\i86Win32VS2010\%TEST_NAME%Client.exe"
set errorstatus=%ERRORLEVEL%
:: Kill server
...
msbuild "%TEST_NAME%-i86Win32VS2010.sln" /t:Clean
:: Build the visual solution
msbuild "%TEST_NAME%-i86Win32VS2010.sln" /t:Build /p:Configuration="Debug DLL"
set errorstatus=%ERRORLEVEL%
if not %errorstatus%==0 goto :exit
:: Execute the server in other cmd.exe
start objs\i86Win32VS2010\%TEST_NAME%Server.exe
...
msbuild "%TEST_NAME%-i86Win32VS2010.sln" /t:Clean
:: Build the visual solution
msbuild "%TEST_NAME%-i86Win32VS2010.sln" /t:Build /p:Configuration="Release"
set errorstatus=%ERRORLEVEL%
if not %errorstatus%==0 goto :exit
:: Execute the server in other cmd.exe
start objs\i86Win32VS2010\%TEST_NAME%Server.exe
