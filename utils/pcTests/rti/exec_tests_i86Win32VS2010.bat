...
del *.h *.cxx %*RequestReply.idl *-i86Win32VS2010*
:: Generates the file with RPCDDS script
call %RPCDDSHOME%\scripts\rpcdds_rti_pcTests.bat -ppDisable -example i86Win32VS2010 "%*.idl"
...
msbuild "%*-i86Win32VS2010.sln" /t:Clean
:: Build the visual solution
msbuild "%*-i86Win32VS2010.sln" /t:Build /p:Configuration="Release DLL"
set errorstatus=%ERRORLEVEL%
if not %errorstatus%==0 goto :EOF
:: Execute the server in other cmd.exe
start objs\i86Win32VS2010\%*Server.exe
:: Wait 5 seconds
call :wait 5
:: Execute the client in this cmd.exe
"objs\i86Win32VS2010\%*Client.exe"
set errorstatus=%ERRORLEVEL%
:: Kill server
...
msbuild "%*-i86Win32VS2010.sln" /t:Clean
:: Build the visual solution
msbuild "%*-i86Win32VS2010.sln" /t:Build /p:Configuration="Debug DLL"
set errorstatus=%ERRORLEVEL%
if not %errorstatus%==0 goto :EOF
:: Execute the server in other cmd.exe
start objs\i86Win32VS2010\%*Server.exe
...
msbuild "%*-i86Win32VS2010.sln" /t:Clean
:: Build the visual solution
msbuild "%*-i86Win32VS2010.sln" /t:Build /p:Configuration="Release"
set errorstatus=%ERRORLEVEL%
if not %errorstatus%==0 goto :EOF
:: Execute the server in other cmd.exe
start objs\i86Win32VS2010\%*Server.exe
...
msbuild "%*-i86Win32VS2010.sln" /t:Clean
:: Build the visual solution
msbuild "%*-i86Win32VS2010.sln" /t:Build /p:Configuration="Debug"
set errorstatus=%ERRORLEVEL%
if not %errorstatus%==0 goto :EOF
:: Execute the server in other cmd.exe
start objs\i86Win32VS2010\%*Server.exe
