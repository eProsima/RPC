...
def generate_code():
    ...
    check_call([java_executable, "-jar", project_source_dir + datadir + "/rpcddsgen.jar", "-local", "-replace", 
        "-topicGeneration", topic, "-d", output_dir, "-example", example,
...
def execute_test():
    ...
    if platform.system() == "Linux":
        ...
        if libdir != None:
            envvars["LD_LIBRARY_PATH"] =  libdir.replace(";", ":")
        else:
            envvars["LD_LIBRARY_PATH"] = project_source_dir + "/lib"
