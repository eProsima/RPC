...
definition returns [Pair<Definition, TemplateGroup> dtg = null]
{
	...
	:   (   tdtg=type_dcl SEMI! {dtg = new Pair<Definition, TemplateGroup>(tdtg.first(), tdtg.second());}
	    |   const_dcl SEMI!
	    |   except_dcl SEMI!
	    |   (("abstract" | "local")? "interface") => itg=interf SEMI! {dtg = new Pair<Definition, TemplateGroup>(itg.first(), itg.second());}
	    |   mtg=module SEMI! {dtg = new Pair<Definition, TemplateGroup>(mtg.first(), mtg.second());}
