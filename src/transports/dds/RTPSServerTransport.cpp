...
transport::Endpoint* RTPSServerTransport::createProcedureEndpoint(
        const char* name,
        ...
        size_t dataSize)
{
    ...
    if (pe != NULL)
    {
        if (pe->initialize(name, writertypename, writertopicname, readertypename, readertopicname, create_data,
                destroy_data, processFunc, dataSize) == 0)
        {
            std::pair<std::map<const char*, RTPSServerProcedureEndpoint*>::iterator,
                    bool> retmap = m_procedureEndpoints.insert(std::pair<const char*,
                            RTPSServerProcedureEndpoint*>(name, pe));
...
void RTPSServerTransport::process(
...
void RTPSServerTransport::sendReply(
        void* data,
        size_t /*dataLength*/,
...
int RTPSServerTransport::receive(
        char* /*buffer*/,
        size_t /*bufferLength*/,
        size_t& /*dataToRead*/,
        transport::Endpoint* /*endpoint*/)
